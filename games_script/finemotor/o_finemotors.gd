# OCTOPUS TO FISHERMAN GAME CONTROLLER
...
var octopus_caught := 0
var total_octopus := 5  # Octopus1–Octopus5

# Node references
@onready var timer_display = $Time/Label
@onready var draggables = $Draggables  # Contains all 5 octopus nodes
@onready var fisherman = $Dropzone/Fisherman  # Single dropzone (Fisherman)
...
var placed_octopus := []  # Track caught octopus
...
func check_drop(item: Control, drop_position: Vector2):
	if fisherman.get_global_rect().has_point(drop_position):
...
func handle_correct_drop(item: Control):
	...
	# Calculate position for row placement near fisherman
	var fish_rect = fisherman.get_global_rect()
	var spacing = 45
	var start_x = fish_rect.position.x + fish_rect.size.x / 2 - (total_octopus * spacing) / 2
	var y_position = fish_rect.position.y + fish_rect.size.y / 1.5

	var index = placed_octopus.size()
	var target_pos = Vector2(start_x + index * spacing, y_position)
	placed_octopus.append(item)

	# Animate to fisherman and shrink
	var tween = create_tween()
	tween.tween_property(item, "global_position", target_pos, 0.4)
	tween.tween_property(item, "scale", Vector2(0.45, 0.45), 0.3)
	await tween.finished
	item.visible = false

	if octopus_caught >= total_octopus:
