...
var watermelons_given := 0
var required_watermelons := 3
...
# Lady messages
var lady_messages = {
	"welcome": "Give me exactly 3 watermelons, please!",
	"correct_count": "Perfect! Thank you for the 3 watermelons!",
	"too_many": "Whoa! I only need 3 watermelons, not more!",
	"need_more": "I still need more watermelons... %d more to go!",
...
func handle_item_drop_on_lady(item: Control):
	...
	watermelons_with_lady.append(item)
	watermelons_given += 1
	position_item_with_lady(item)
	evaluate_watermelon_count()
...
func return_item_from_lady(item: Control):
	if item in watermelons_with_lady:
		watermelons_with_lady.erase(item)
		watermelons_given -= 1
	item.visible = true
	item.mouse_filter = Control.MOUSE_FILTER_PASS
	return_to_original_position(item)
	if watermelons_given < required_watermelons:
		var remaining = required_watermelons - watermelons_given
...
func evaluate_watermelon_count():
	if watermelons_given > required_watermelons:
		update_lady_message("too_many")
		await get_tree().create_timer(2.0).timeout
		return_all_watermelons()
		update_lady_message("welcome")
	elif watermelons_given == required_watermelons:
...
func return_all_watermelons():
	if win_timer.time_left > 0:
		win_timer.stop()
	for item in watermelons_with_lady.duplicate():
		return_item_from_lady(item)
	watermelons_with_lady.clear()
	watermelons_given = 0
