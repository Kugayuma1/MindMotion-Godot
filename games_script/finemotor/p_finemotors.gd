# PEPPERONI PIZZA GAME CONTROLLER
...
var total_pepperoni := 5  # 5 draggables: Pepperoni1–5

# Node references
@onready var timer_display = $Time/Label
@onready var draggables = $Draggables  # Contains all pepperoni slices
@onready var pizza = $Dropzone/Pizza  # Dropzone node
...
var placed_positions := [] # To help randomize positions better
...
func check_drop(item: Control, drop_position: Vector2):
	if pizza.get_global_rect().has_point(drop_position):
		if item.name.begins_with("Pepperoni"):  # ✅ Correct item
...
func handle_correct_drop(item: Control):
	pepperoni_collected += 1
	item.mouse_filter = Control.MOUSE_FILTER_IGNORE

	var pizza_rect = pizza.get_global_rect()

	# Generate random position within pizza bounds (avoiding overlap)
	var target_pos = get_random_non_overlapping_position(pizza_rect, 50)
	...
	if pepperoni_collected >= total_pepperoni:
		await get_tree().create_timer(0.5).timeout
		win_game()

func get_random_non_overlapping_position(pizza_rect: Rect2, min_distance: float) -> Vector2:
	var attempts = 0
	while attempts < 50:
		var rand_x = randf_range(pizza_rect.position.x + 40, pizza_rect.position.x + pizza_rect.size.x - 40)
		var rand_y = randf_range(pizza_rect.position.y + 40, pizza_rect.position.y + pizza_rect.size.y - 40)
	...
	return Vector2(
		randf_range(pizza_rect.position.x + 40, pizza_rect.position.x + pizza_rect.size.x - 40),
		randf_range(pizza_rect.position.y + 40, pizza_rect.position.y + pizza_rect.size.y - 40)
	)
