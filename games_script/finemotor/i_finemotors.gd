...
var insects_caught := 0
var total_insects := 6  # Insect1–Insect6

# Node references
@onready var timer_display = $Time/Label
@onready var draggables = $Draggables  # Contains Insect1, Insect2, ...
@onready var jar = $Dropzone/Jar       # Single dropzone
...
func check_drop(item: Control, drop_position: Vector2):
	if jar.get_global_rect().has_point(drop_position):
		handle_correct_drop(item)
	else:
		# There’s no wrong answer — just return the insect to its original position
...
func handle_correct_drop(item: Control):
	...
	# Random position within jar center region
	var jar_rect = jar.get_global_rect()
	var center_area = Rect2(
		jar_rect.position + jar_rect.size * 0.25,  # Start 25% inside the jar
		jar_rect.size * 0.5                        # Middle 50% area
	)

	var random_x = randf_range(center_area.position.x, center_area.position.x + center_area.size.x - item.size.x)
	var random_y = randf_range(center_area.position.y, center_area.position.y + center_area.size.y - item.size.y)
	var random_position = Vector2(random_x, random_y)

	# Animate insect to jar position
	var tween = create_tween()
	tween.tween_property(item, "global_position", random_position, 0.4)
	tween.tween_property(item, "scale", Vector2(0.7, 0.7), 0.3)

	if insects_caught >= total_insects:
