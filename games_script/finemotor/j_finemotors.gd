# JEWEL TO TREASUREBOX GAME CONTROLLER
...
var jewels_placed := 0
var total_jewels := 4  # Jewel1–Jewel4

# Node references
@onready var timer_display = $Time/Label
@onready var draggables = $Draggables  # Contains Jewel1–Jewel4
@onready var treasure_box = $Dropzone/TreasureBox  # Single dropzone
...
func check_drop(item: Control, drop_position: Vector2):
	if treasure_box.get_global_rect().has_point(drop_position):
		handle_correct_drop(item)
	else:
		# No wrong answer — return to original if missed
		return_to_original_position(item)

func handle_correct_drop(item: Control):
	if item in placed_jewels:
		return  # already placed

	jewels_placed += 1
	placed_jewels.append(item)
	item.mouse_filter = Control.MOUSE_FILTER_IGNORE

	# Arrange jewels in a closer row inside the treasure box
	var box_rect = treasure_box.get_global_rect()
	var spacing = (box_rect.size.x * 0.35) / (total_jewels - 1)  # closer together (was 0.55 before)
	var start_x = box_rect.position.x + (box_rect.size.x * 0.325)  # centered adjustment
	var y_pos = box_rect.position.y + (box_rect.size.y * 0.55) - (item.size.y * 0.3)  # slightly lower center

	var target_x = start_x + spacing * (jewels_placed - 1)
	var target_position = Vector2(target_x, y_pos)

	# Animate jewel into position and shrink nicely
	var tween = create_tween()
	tween.tween_property(item, "global_position", target_position, 0.4)
	tween.tween_property(item, "scale", Vector2(0.6, 0.6), 0.3)  # fits inside treasure box

	if jewels_placed >= total_jewels:
