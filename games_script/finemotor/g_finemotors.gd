...
var tools_given := 0
var total_tools := 3  # Gardentools, Gardentools2, Gardentools3

# Node references
@onready var timer_display = $Time/Label
@onready var draggables = $Draggables
@onready var gardener = $Dropzone/Gardener
...
func check_drop(item: Control, drop_position: Vector2):
	if gardener.get_global_rect().has_point(drop_position):
		if item.name.to_lower().begins_with("gardentools"):
