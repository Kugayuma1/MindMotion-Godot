# YELLOW TOY TO KID GAME CONTROLLER
...
var total_toys := 4  # YellowToy, YellowToy2, YellowToy3, YellowToy4

# Node references
@onready var timer_display = $Time/Label
@onready var draggables = $Draggables  # Contains both toys and decoys
@onready var kid = $Dropzone/Kid  # Dropzone
...
func setup_game():
	...
	# Connect all draggables (toys and decoys)
...
func check_drop(item: Control, drop_position: Vector2):
	if kid.get_global_rect().has_point(drop_position):
		if item.name.begins_with("YellowToy"):  # ✅ Correct item
			handle_correct_drop(item)
		else:
			return_to_original_position(item)  # ❌ Decoy returns
	else:
...
func handle_correct_drop(item: Control):
	...
	# Place neatly in a row on the Kid
	var kid_rect = kid.get_global_rect()
	var spacing = 45
	var start_x = kid_rect.position.x + kid_rect.size.x / 2 - (total_toys * spacing) / 2
	var y_position = kid_rect.position.y + kid_rect.size.y / 1.6

	var index = placed_toys.size()
	var target_pos = Vector2(start_x + index * spacing, y_position)
	placed_toys.append(item)

	# Animate move and shrink
	var tween = create_tween()
	tween.tween_property(item, "global_position", target_pos, 0.4)
	tween.tween_property(item, "scale", Vector2(0.3, 0.3), 0.3)
	await tween.finished
	item.visible = false

	if toys_collected >= total_toys:
		await get_tree().create_timer(0.5).timeout
		win_game()

# Bounce back for wrong or misplaced items
