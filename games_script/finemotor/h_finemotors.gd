# HEART TO CUPID GAME CONTROLLER
...
var hearts_sorted := 0
var total_hearts := 4  # BlueHeart, RedH, PurpleH, PinkH

# Node references
@onready var timer_display = $Time/Label
@onready var draggables = $Draggables  # Contains BlueHeart, RedH, PurpleH, PinkH
@onready var dropzones = $Dropzone     # Contains CupidBlue, CupidRed, CupidPurple, CupidPink
...
func setup_game():
	...
	# Store draggable nodes and original positions
...
func check_drop(item: Control, drop_position: Vector2):
	for cupid in dropzones.get_children():
		if cupid.get_global_rect().has_point(drop_position):
			if is_match(item, cupid):
...
func is_match(item: Control, cupid: Control) -> bool:
	# Match hearts to corresponding cupids by color name
	if item.name.to_lower().contains("blue") and cupid.name.to_lower().contains("blue"):
		return true
	if item.name.to_lower().contains("red") and cupid.name.to_lower().contains("red"):
		return true
	if item.name.to_lower().contains("purple") and cupid.name.to_lower().contains("purple"):
		return true
	if item.name.to_lower().contains("pink") and cupid.name.to_lower().contains("pink"):
		return true
	return false
