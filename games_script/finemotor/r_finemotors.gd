# ROCKET TO COLOR PAD GAME CONTROLLER
# Attach this script to your main Control node
extends Control
...
var rockets_placed := 0
var total_rockets := 4  # PurpleRocket, WhiteRocket, BlueRocket, RedRocket

# Node references
@onready var timer_display = $Time/Label
@onready var draggables = $Draggables  # Contains all 4 rockets
@onready var dropzones = $Dropzone     # Contains 4 zones: Purple, White, Blue, Red
...
func check_drop(item: Control, drop_position: Vector2):
	for zone in dropzones.get_children():
		if zone.get_global_rect().has_point(drop_position):
			if is_correct_drop(item, zone):
				handle_correct_drop(item, zone)
				return
	return_to_original_position(item)

func is_correct_drop(item: Control, zone: Control) -> bool:
	match item.name:
		"PurpleRocket": return zone.name == "Purple"
		"WhiteRocket": return zone.name == "White"
		"BlueRocket": return zone.name == "Blue"
		"RedRocket": return zone.name == "Red"
		_: return false

func handle_correct_drop(item: Control, zone: Control):
	rockets_placed += 1
	item.mouse_filter = Control.MOUSE_FILTER_IGNORE

	# Target position - center of the dropzone
	var zone_rect = zone.get_global_rect()
	var target_pos = zone_rect.position + zone_rect.size / 2 - item.size / 2

	# Animate to dropzone
	var tween = create_tween()
	tween.tween_property(item, "global_position", target_pos, 0.3)
	tween.tween_property(item, "scale", Vector2(0.6, 0.6), 0.3)
	await tween.finished

	# Disappear smoothly
	var fade_tween = create_tween()
	fade_tween.tween_property(item, "modulate:a", 0.0, 0.3)
	await fade_tween.finished
	item.visible = false

	if rockets_placed >= total_rockets:
