...
var total_toys := 5  # 5 draggables: Toy1–Toy5

# Node references
@onready var timer_display = $Time/Label
@onready var draggables = $Draggables  # Contains all toy nodes
@onready var toybox = $Dropzone/Toybox  # Dropzone node
...
func check_drop(item: Control, drop_position: Vector2):
	if toybox.get_global_rect().has_point(drop_position):
		if item.name.begins_with("Toy"):  # ✅ Correct item
...
func handle_correct_drop(item: Control):
	...
	# Fade out and disable the toy to simulate being "placed inside"
	var tween = create_tween()
	tween.tween_property(item, "modulate:a", 0.0, 0.4)
