# Z_Arts Control Script with Popup System
extends Control
...
var is_mobile := false  # NEW: Track if running on mobile
...
func _ready():
	# NEW: Detect if running on mobile
	...
	# Debug info
	print("Z_Arts Game Manager Ready!")
	print("Target Node (Node2D2): ", target_node)
	print("Running on mobile: ", is_mobile)
	if target_node:
...
# MODIFIED: Added wrong color detection and feedback
func check_drop_target():
	...
	if topmost_polygon:
		...
		if result:
			# Check if the color applied is correct
			if topmost_polygon.has_method("is_correctly_colored") and not topmost_polygon.is_correctly_colored():
				# Wrong color applied - trigger feedback
				trigger_wrong_color_feedback(topmost_polygon)
			else:
				# Correct color or doesn't have validation method
				call_deferred("check_win_condition")

# NEW: Haptic feedback for wrong color
func trigger_wrong_color_feedback(polygon: Polygon2D):
	# Trigger vibration on mobile devices
	if is_mobile:
		Input.vibrate_handheld(200)  # Vibrate for 200 milliseconds
	else:
		# Desktop feedback - print to console for debugging
		print("📳 VIBRATION: Wrong color applied to ", polygon.name)
	
	# Show visual feedback on the polygon
	if polygon.has_method("show_wrong_color_feedback"):
		polygon.show_wrong_color_feedback()
	
	# Optional: Show temporary message on screen
	show_temporary_message("❌ Wrong color!", Color.ORANGE)
	
	# Optional: Time penalty (uncomment if you want this)
	# countdown = max(0, countdown - 1)  # Deduct 1 second
	# if timer_label:
	#	timer_label.text = "⏱️ " + str(countdown) + "s"

# NEW: Show temporary feedback message
func show_temporary_message(message: String, color: Color, duration: float = 1.0):
	if main_label and not game_completed:
...
# NEW: Shake effect for extra feedback
func shake_label():
	if main_label:
		var original_pos = main_label.position
		var tween = create_tween()
		tween.tween_property(main_label, "position", original_pos + Vector2(randf_range(-3, 3), 0), 0.05)
		tween.tween_property(main_label, "position", original_pos + Vector2(randf_range(-3, 3), 0), 0.05)
		tween.tween_property(main_label, "position", original_pos, 0.05)
...
func game_over(success: bool):
	print("Z_Arts game over called with success: ", success)
	...
	if popup_instance:
		...
		print("Z_Arts popup instantiated and added to scene")
...
func force_win_check():
	print("🔍 FORCING Z_ARTS WIN CHECK...")
	check_win_condition()
