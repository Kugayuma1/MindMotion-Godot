# G_Arts Control Script with Popup System
extends Control
...
func _ready():
	...
	# Debug info
	print("G_Arts Game Manager Ready!")
...
func update_timer() -> void:
	...
	if countdown <= 0:
		...
		ProgressManager.save_progress("gart", false)
		Global.refresh_everything_after_stage_completion("gart", false)
...
func win_game():
	...
	ProgressManager.save_progress("gart", true)
	Global.refresh_everything_after_stage_completion("gart", true)
	# Show appropriate popup based on completion time
	game_over(true)

# ---------------- POPUP SYSTEM ----------------
func game_over(success: bool):
	print("G_Arts game over called with success: ", success)
	...
	if popup_instance:
		...
		print("G_Arts popup instantiated and added to scene")
...
func force_win_check():
	print("🔍 FORCING G_ARTS WIN CHECK...")
	check_win_condition()
