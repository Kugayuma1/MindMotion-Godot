# A_Arts Control Script with Popup System
extends Control
...
@onready var main_label = $label_main
@onready var target_node = $Node2D2
...
func _ready():
	...
	print("A_Arts Game Manager Ready!")
...
func game_over(success: bool):
	print("A_Arts game over called with success: ", success)
	hide_game_elements()
	if success:
		if countdown >= 10:
	...
	if popup_instance:
		...
		print("A_Arts popup instantiated and added to scene")
...
func force_win_check():
	print("🔍 FORCING A_ARTS WIN CHECK...")
	check_win_condition()
