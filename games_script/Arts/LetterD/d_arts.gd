# D_Arts Control Script with Popup System
extends Control
...
func _ready():
	...
	# Debug info
	print("D_Arts Game Manager Ready!")
...
func update_timer() -> void:
	...
	if countdown <= 0:
		...
		ProgressManager.save_progress("dart", false)
		Global.refresh_everything_after_stage_completion("dart", false)
...
func win_game():
	...
	ProgressManager.save_progress("dart", true)
	Global.refresh_everything_after_stage_completion("dart", true)
	# Show appropriate popup based on completion time
	game_over(true)

# ---------------- POPUP SYSTEM ----------------
func game_over(success: bool):
	print("D_Arts game over called with success: ", success)
	...
	if popup_instance:
		...
		print("D_Arts popup instantiated and added to scene")
...
func force_win_check():
	print("🔍 FORCING D_ARTS WIN CHECK...")
	check_win_condition()
