...
func _ready() -> void:
	feedback_label.text = "🔍The one that cleans your Teeth"
	start_timer()
...
func check_answer(choice: String, node: Control) -> void:
	...
	if choice == correct_answer:
		timer_active = false
		feedback_label.text = "✅ Correct! Toothbrush to clean your teeth"
		...
		feedback_label.text = "Cleans your teeth?"
...
func _on_toothbrush_pressed() -> void:
	check_answer("Toothbrush", $Toothbrush)

func _on_shampoo_pressed() -> void:
	check_answer("Shampoo", $Shampoo)

func _on_soap_pressed() -> void:
	check_answer("Soap", $Soap)
