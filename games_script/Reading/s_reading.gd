...
var total_required_matches := 3  # Man, Eating, Sandwich dropzones
...
func is_correct_match(item_name: String, zone_name: String) -> bool:
	# The correct answer based on the phrase: "The Man is eating a Sandwich"
	# Match specific pairs:
	# "Man" draggable -> "ManDrop" dropzone
	# "Eating" draggable -> "EatingDrop" dropzone
	# "Sandwich" draggable -> "SandwichDrop" dropzone
	...
	if "man" in clean_item and "mandrop" in clean_zone:
		return true
	elif "eating" in clean_item and "eatingdrop" in clean_zone:
		return true
	elif "sandwich" in clean_item and "sandwichdrop" in clean_zone:
		return true
