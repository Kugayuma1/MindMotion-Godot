...
var total_required_matches := 3  # Girl, Eating, Ice Cream dropzones
...
func is_correct_match(item_name: String, zone_name: String) -> bool:
	# The correct answer based on the phrase: "The Girl is eating Ice Cream"
	# Match specific pairs:
	# "Girl" draggable -> "GirlDrop" dropzone
	# "Eating" draggable -> "EatingDrop" dropzone
	# "Ice Cream" draggable -> "IceCreamDrop" dropzone
	...
	if "girl" in clean_item and "girldrop" in clean_zone:
		return true
	elif "eating" in clean_item and "eatingdrop" in clean_zone:
		return true
	elif "icecream" in clean_item and "icecreamdrop" in clean_zone:
		return true
