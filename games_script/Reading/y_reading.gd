...
var total_required_matches := 3  # Lady, Eating, Yogurt dropzones
...
func is_correct_match(item_name: String, zone_name: String) -> bool:
	# The correct answer based on the phrase: "The Lady is eating Yogurt"
	# Match specific pairs:
	# "Lady" draggable -> "LadyDrop" dropzone
	# "Eating" draggable -> "EatingDrop" dropzone
	# "Yogurt" draggable -> "YogurtDrop" dropzone
	...
	if "lady" in clean_item and "ladydrop" in clean_zone:
		return true
	elif "eating" in clean_item and "eatingdrop" in clean_zone:
		return true
	elif "yogurt" in clean_item and "yogurtdrop" in clean_zone:
		return true
