...
func _ready() -> void:
	feedback_label.text = "🔍The one that eats grass"
	start_timer()
...
func check_answer(choice: String, node: Control) -> void:
	...
	if choice == correct_answer:
		timer_active = false
		feedback_label.text = "✅ Correct! Horse is the on who's eating grass"
		...
		feedback_label.text = "Eats grass?"
...
func _on_bear_pressed() -> void:
	check_answer("Bear", $Bear)


func _on_horse_pressed() -> void:
	check_answer("Horse", $Horse)


func _on_rat_pressed() -> void:
	check_answer("Rat", $Rat)
