...
func _ready() -> void:
	feedback_label.text = "🔍The one who likes banana!"
	start_timer()
...
func check_answer(choice: String, node: Control) -> void:
	...
	if choice == correct_answer:
		timer_active = false
		feedback_label.text = "✅ Correct! Monkey is the one who likes banana"
		...
		feedback_label.text = "Likes banana?"
...
func _on_monkey_pressed() -> void:
	check_answer("Monkey", $Monkey)

func _on_lizard_pressed() -> void:
	check_answer("Lizard", $Lizard)


func _on_zebra_pressed() -> void:
	check_answer("Zebra", $Zebra)
