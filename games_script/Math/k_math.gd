...
# Lady messages
var lady_messages = {
	"welcome": "Give me exactly 3 kiwis, please!",
	"correct_count": "Perfect! Thank you for the 3 kiwis!",
	"too_many": "Whoa! I only need 3 kiwis, not more!",
	"need_more": "I still need more kiwis... %d more to go!",
...
func return_item_from_lady(item: Control):
	if item in kiwis_with_lady:
		kiwis_with_lady.erase(item)
		kiwis_given -= 1
	item.visible = true
	item.mouse_filter = Control.MOUSE_FILTER_PASS
	return_to_original_position(item)
	if kiwis_given < required_kiwis:
		var remaining = required_kiwis - kiwis_given
...
func evaluate_kiwi_count():
	if kiwis_given > required_kiwis:
		update_lady_message("too_many")
		await get_tree().create_timer(2.0).timeout
		return_all_kiwis()
		update_lady_message("welcome")
	elif kiwis_given == required_kiwis:
...
func return_all_kiwis():
	if win_timer.time_left > 0:
		win_timer.stop()
	for item in kiwis_with_lady.duplicate():
		return_item_from_lady(item)
	kiwis_with_lady.clear()
	kiwis_given = 0
