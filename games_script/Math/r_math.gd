...
# Lady messages
var lady_messages = {
	"welcome": "Give me exactly 4 roses, please!",
	"correct_count": "Perfect! Thank you for the 4 roses!",
	"too_many": "Whoa! I only need 4 roses, not more!",
	"need_more": "I still need more roses... %d more to go!",
...
func return_item_from_lady(item: Control):
	if item in roses_with_lady:
		roses_with_lady.erase(item)
		roses_given -= 1
	item.visible = true
	item.mouse_filter = Control.MOUSE_FILTER_PASS
	return_to_original_position(item)
	if roses_given < required_roses:
		var remaining = required_roses - roses_given
...
func evaluate_rose_count():
	if roses_given > required_roses:
		update_lady_message("too_many")
		await get_tree().create_timer(2.0).timeout
		return_all_roses()
		update_lady_message("welcome")
	elif roses_given == required_roses:
...
func return_all_roses():
	if win_timer.time_left > 0:
		win_timer.stop()
	for item in roses_with_lady.duplicate():
		return_item_from_lady(item)
	roses_with_lady.clear()
	roses_given = 0
