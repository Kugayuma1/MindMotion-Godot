...
@onready var man_zone = $Dropzone/Man
@onready var man_label = $Dropzone/Man/Label2
...
# Man messages
var man_messages = {
	"welcome": "Give me exactly 4 wheels, please!",
	"correct_count": "Perfect! Thank you for the 4 wheels!",
	"too_many": "Whoa! I only need 4 wheels, not more!",
	"need_more": "I still need more wheels... %d more to go!",
...
func end_drag(item: Control, mouse_pos: Vector2):
	...
	check_drop_on_man(item, mouse_pos)

func check_drop_on_man(item: Control, drop_position: Vector2):
	if man_zone and man_zone.get_global_rect().has_point(drop_position):
		handle_item_drop_on_man(item)
	else:
		if item in wheels_with_man:
			return_item_from_man(item)
		else:
			return_to_original_position(item)

func handle_item_drop_on_man(item: Control):
	if not item.name.to_lower().begins_with("wheel"):
		return_to_original_position(item)
		return
	...
	wheels_with_man.append(item)
	wheels_given += 1
	position_item_with_man(item)
	evaluate_wheel_count()

func position_item_with_man(item: Control):
	item.visible = false
	item.mouse_filter = Control.MOUSE_FILTER_IGNORE

func return_item_from_man(item: Control):
	if item in wheels_with_man:
		wheels_with_man.erase(item)
		wheels_given -= 1
	item.visible = true
	item.mouse_filter = Control.MOUSE_FILTER_PASS
	return_to_original_position(item)
	if wheels_given < required_wheels:
		var remaining = required_wheels - wheels_given
		update_man_message("need_more", [remaining])
...
func evaluate_wheel_count():
	if wheels_given > required_wheels:
		update_man_message("too_many")
		await get_tree().create_timer(2.0).timeout
		return_all_wheels()
		update_man_message("welcome")
	elif wheels_given == required_wheels:
		update_man_message("correct_count")
		win_game()
	else:
		var remaining = required_wheels - wheels_given
		update_man_message("need_more", [remaining])

func return_all_wheels():
	if win_timer.time_left > 0:
		win_timer.stop()
	for item in wheels_with_man.duplicate():
		return_item_from_man(item)
	wheels_with_man.clear()
	wheels_given = 0
...
func update_man_message(message_key: String, args: Array = []):
	if man_label:
		var message = man_messages[message_key]
		if args.size() > 0:
			message = message % args
		man_label.text = message
