# VISUAL MATH GAME - RANDOMIZED SUBTRACTION FOR VOLLEYBALL SCENE
...
# Volleyball group paths - 7 TOTAL VOLLEYBALLS (5 left, 2 right)
...
func initialize_game() -> void:
	...
	display_volleyballs_based_on_random()
	setup_math_problem()
	setup_ui()
...
func generate_random_numbers() -> void:
	"""Generate random volleyball counts ensuring left >= right for positive subtraction"""
	...
	print("Random left volleyballs: %d" % random_left_count)
	print("Random right volleyballs: %d" % random_right_count)
...
func collect_volleyball_nodes() -> void:
	"""Collect all volleyball nodes from both groups"""
	left_volleyballs.clear()
	right_volleyballs.clear()
	
	for volleyball_path in left_volleyball_paths:
		var volleyball_node = get_node_or_null(volleyball_path)
		if volleyball_node:
			left_volleyballs.append(volleyball_node)
			print("Found left volleyball: %s" % volleyball_path)
		else:
			print("Missing left volleyball: %s" % volleyball_path)
	
	for volleyball_path in right_volleyball_paths:
		var volleyball_node = get_node_or_null(volleyball_path)
		if volleyball_node:
			right_volleyballs.append(volleyball_node)
			print("Found right volleyball: %s" % volleyball_path)
		else:
			print("Missing right volleyball: %s" % volleyball_path)
	
	print("Left volleyballs: %d, Right volleyballs: %d" % [left_volleyballs.size(), right_volleyballs.size()])

func display_volleyballs_based_on_random() -> void:
	"""Show only the random amount of volleyballs in shuffled positions"""
	# Collect all left volleyball indices
	var left_indices = []
	for i in range(left_volleyballs.size()):
		left_indices.append(i)
	left_indices.shuffle()
	
	# Collect all right volleyball indices
	var right_indices = []
	for i in range(right_volleyballs.size()):
		right_indices.append(i)
	right_indices.shuffle()
	
	# Hide all volleyballs first
	for volleyball in left_volleyballs:
		volleyball.visible = false
	for volleyball in right_volleyballs:
		volleyball.visible = false
	
	# Show only the random count from shuffled positions
	for i in range(random_left_count):
		if i < left_indices.size():
			left_volleyballs[left_indices[i]].visible = true
	
	for i in range(random_right_count):
		if i < right_indices.size():
			right_volleyballs[right_indices[i]].visible = true
	
	print("Displayed %d left volleyballs (shuffled) and %d right volleyballs (shuffled)" % [random_left_count, random_right_count])

# === MATH PROBLEM SETUP ===
func setup_math_problem() -> void:
	"""Create the math problem based on visible volleyball counts"""
...
func setup_ui() -> void:
	...
	if equation_label:
		equation_label.text = "Solve the volleyball: " + current_equation
