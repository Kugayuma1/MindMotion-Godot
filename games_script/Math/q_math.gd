...
var current_question := "How many Quail are in the scene?"
...
# Quail container
@onready var quail_container = $Quail  

# List of Quail node names (fallback if container not used)
var quail_names = ["Quail1", "Quail2", "Quail3", "Quail4", "Quail5", "Quail6", "Quail7", "Quail8"]
...
func find_all_quail():
	...
	if quail_container:
		for child in quail_container.get_children():
			quail_nodes.append(child)
			print("✅ Found quail in container: %s" % child.name)
	else:
		# Method 2: From predefined names
		for quail_name in quail_names:
			var quail_node = get_node_or_null(quail_name)
			if quail_node:
				quail_nodes.append(quail_node)
				print("✅ Found quail by name: %s" % quail_name)
			else:
				print("❌ Could not find quail: %s" % quail_name)
	
	print("Total quails found: %d" % quail_nodes.size())
	
	if quail_nodes.size() == 0:
		print("⚠️ No quails found! Using fallback count of 8")
		correct_answer = 8
	else:
		correct_answer = quail_nodes.size()

func setup_counting_game():
	if question_label:
		question_label.text = current_question
	print("Question: %s" % current_question)
	print("Correct answer: %d Quails" % correct_answer)
...
func _on_button_pressed(button_num: int):
	...
	if selected_answer == correct_answer:
		if question_label:
			question_label.text = "✅ Correct! There are " + str(correct_answer) + " Quails!"
		timer_active = false
		game_over(true)
	else:
		if question_label:
...
func update_timer() -> void:
	if countdown <= 0:
		timer_active = false
		if question_label:
			question_label.text = "⏱️ Time's up! Answer: " + str(correct_answer) + " Quails"
		game_over(false)
		return
...
func game_over(success: bool):
	...
	if has_node("Quail"): $Quail.visible = false
