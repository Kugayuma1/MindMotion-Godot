# VISUAL MATH GAME - CLEAN VERSION FOR NECKLACE SCENE
...
# === INITIALIZATION ===
func _ready() -> void:
	print("📿 Starting Necklace Math Game - Debug Mode")
...
func collect_necklace_nodes() -> void:
	...
	for necklace_path in left_necklace_paths:
		var node = get_node_or_null(necklace_path)
		if node: left_necklaces.append(node)
	for necklace_path in right_necklace_paths:
		var node = get_node_or_null(necklace_path)
		if node: right_necklaces.append(node)
	print("📿 Left necklaces: %d, Right necklaces: %d" % [left_necklaces.size(), right_necklaces.size()])

# === MATH PROBLEM SETUP ===
func setup_math_problem() -> void:
	var left_count = left_necklaces.size()
	var right_count = right_necklaces.size()
