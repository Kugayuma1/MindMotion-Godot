# VISUAL MATH GAME - CLEAN VERSION FOR YARN SCENE
...
# === INITIALIZATION ===
func _ready() -> void:
	print("🧶 Starting Yarn Math Game - Debug Mode")
...
func collect_yarn_nodes() -> void:
	...
	for yarn_path in left_yarn_paths:
		var node = get_node_or_null(yarn_path)
		if node: left_yarns.append(node)
	for yarn_path in right_yarn_paths:
		var node = get_node_or_null(yarn_path)
		if node: right_yarns.append(node)
	print("🧶 Left yarns: %d, Right yarns: %d" % [left_yarns.size(), right_yarns.size()])

# === MATH PROBLEM SETUP ===
func setup_math_problem() -> void:
	var left_count = left_yarns.size() # 6
	var right_count = right_yarns.size() # 3
