...
var current_question := "How many Money are in the scene?"
...
# Money container
@onready var money_container = $Money  

# List of Money node names (fallback if container not used)
var money_names = ["Money1", "Money2", "Money3", "Money4", "Money5"]
...
func find_all_money():
	...
	if money_container:
		for child in money_container.get_children():
			money_nodes.append(child)
			print("✅ Found money in container: %s" % child.name)
	else:
		# Method 2: From predefined names
		for money_name in money_names:
			var money_node = get_node_or_null(money_name)
			if money_node:
				money_nodes.append(money_node)
				print("✅ Found money by name: %s" % money_name)
			else:
				print("❌ Could not find money: %s" % money_name)
	
	print("Total money found: %d" % money_nodes.size())
	
	if money_nodes.size() == 0:
		print("⚠️ No money found! Using fallback count of 5")
		correct_answer = 5
	else:
		correct_answer = money_nodes.size()

func setup_counting_game():
	if question_label:
		question_label.text = current_question
	print("Question: %s" % current_question)
	print("Correct answer: %d Money" % correct_answer)
...
func _on_button_pressed(button_num: int):
	...
	if selected_answer == correct_answer:
		if question_label:
			question_label.text = "✅ Correct! There are " + str(correct_answer) + " Money!"
		timer_active = false
		game_over(true)
	else:
		if question_label:
...
func update_timer() -> void:
	if countdown <= 0:
		timer_active = false
		if question_label:
			question_label.text = "⏱️ Time's up! Answer: " + str(correct_answer) + " Money"
		game_over(false)
		return
...
func game_over(success: bool):
	...
	if has_node("Money"): $Money.visible = false
