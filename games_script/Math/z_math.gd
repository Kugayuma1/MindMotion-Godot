...
var current_question := "How many Zebras are in the scene?"
...
# Zebra container
@onready var zebra_container = $Zebra  

# List of Zebra node names (fallback if container not used)
var zebra_names = [
	"Zebra1", "Zebra2", "Zebra3", "Zebra4",
	"Zebra5", "Zebra6", "Zebra7", "Zebra8" , "Zebra9"
]
...
func find_all_zebras():
	...
	if zebra_container:
		for child in zebra_container.get_children():
			zebra_nodes.append(child)
			print("✅ Found zebra in container: %s" % child.name)
	else:
		# Method 2: From predefined names
		for zebra_name in zebra_names:
			var zebra_node = get_node_or_null(zebra_name)
			if zebra_node:
				zebra_nodes.append(zebra_node)
				print("✅ Found zebra by name: %s" % zebra_name)
			else:
				print("❌ Could not find zebra: %s" % zebra_name)
	
	print("Total zebras found: %d" % zebra_nodes.size())
	
	if zebra_nodes.size() == 0:
		print("⚠️ No zebras found! Using fallback count of 9")
		correct_answer = 9
	else:
		correct_answer = zebra_nodes.size()

func setup_counting_game():
	if question_label:
		question_label.text = current_question
	print("Question: %s" % current_question)
	print("Correct answer: %d Zebras" % correct_answer)
...
func _on_button_pressed(button_num: int):
	...
	if selected_answer == correct_answer:
		if question_label:
			question_label.text = "✅ Correct! There are " + str(correct_answer) + " Zebras!"
		timer_active = false
		game_over(true)
	else:
		if question_label:
...
func game_over(success: bool):
	...
	if has_node("Zebra"): $Zebra.visible = false
