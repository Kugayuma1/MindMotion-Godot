# VISUAL MATH GAME - FIRE SCENE WITH RANDOMIZATION
...
# Fire group paths - 10 TOTAL FIRES (5 left, 5 right)
...
func generate_random_numbers() -> void:
	"""Generate random fire counts ensuring minimum sum of 7"""
	...
	while not valid_combination:
		...
		# Check if sum is at least 7
		if random_left_count + random_right_count >= min_total_sum:
			valid_combination = true
	
	print("Random left fires: %d" % random_left_count)
	print("Random right fires: %d" % random_right_count)
...
func collect_fire_nodes() -> void:
	...
	for fire_path in left_fire_paths:
		...
		if fire_node:
			...
			print("Missing left fire: %s" % fire_path)
	...
	for fire_path in right_fire_paths:
		...
		if fire_node:
			...
			print("Missing right fire: %s" % fire_path)
	
	print("Left fires: %d, Right fires: %d" % [left_fires.size(), right_fires.size()])

func display_fires_based_on_random() -> void:
	"""Show only the random amount of fires in shuffled positions"""
	# Collect all left fire indices
	var left_indices = []
	for i in range(left_fires.size()):
		left_indices.append(i)
	left_indices.shuffle()
	
	# Collect all right fire indices
	var right_indices = []
	for i in range(right_fires.size()):
	...
	for i in range(random_left_count):
		if i < left_indices.size():
			left_fires[left_indices[i]].visible = true
	
	for i in range(random_right_count):
		if i < right_indices.size():
			right_fires[right_indices[i]].visible = true
	
	print("Displayed %d left fires (shuffled) and %d right fires (shuffled)" % [random_left_count, random_right_count])
...
func setup_ui() -> void:
	...
	if equation_label:
		equation_label.text = "Solve the fire: %d + %d = ?" % [random_left_count, random_right_count]
