# VISUAL MATH GAME - CLEAN VERSION FOR STRAWBERRY SCENE
...
# === INITIALIZATION ===
func _ready() -> void:
	print("🍓 Starting Strawberry Math Game - Debug Mode")
...
func collect_strawberry_nodes() -> void:
	...
	for strawberry_path in left_strawberry_paths:
		var node = get_node_or_null(strawberry_path)
		if node: left_strawberries.append(node)
	for strawberry_path in right_strawberry_paths:
		var node = get_node_or_null(strawberry_path)
		if node: right_strawberries.append(node)
...
func setup_math_problem() -> void:
	var left_count = left_strawberries.size() # 6
	var right_count = right_strawberries.size() # 5
