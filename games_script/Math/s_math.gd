# VISUAL MATH GAME - RANDOMIZED ADDITION FOR STRAWBERRY SCENE
...
# Minimum total addition is 8
var min_addend := 1
var max_addend_left := 6
var max_addend_right := 5
var min_total_sum := 8
...
# Strawberry group paths - 11 TOTAL STRAWBERRIES (6 left, 5 right)
var left_strawberry_paths: Array[String] = [
	"LeftGroup/Strawberry1",
	"LeftGroup/Strawberry2", 
	"LeftGroup/Strawberry3",
	"LeftGroup/Strawberry4",
	"LeftGroup/Strawberry5",
	"LeftGroup/Strawberry6"
]

var right_strawberry_paths: Array[String] = [
	"RightGroup/Strawberry7",
	"RightGroup/Strawberry8",
	"RightGroup/Strawberry9",
	"RightGroup/Strawberry10",
	"RightGroup/Strawberry11"
]

# Strawberry collections
var left_strawberries: Array[Node] = []
var right_strawberries: Array[Node] = []
...
func debug_scene_structure() -> void:
	...
	print("\nChecking strawberry nodes...")
	var all_strawberry_paths = left_strawberry_paths + right_strawberry_paths
	for path in all_strawberry_paths:
		var node = get_node_or_null(path)
		if node:
			print("Found strawberry: %s (%s)" % [path, node.get_class()])
		else:
			print("Missing strawberry: %s" % path)
...
func initialize_game() -> void:
	...
	collect_strawberry_nodes()
	display_strawberries_based_on_random()
	setup_math_problem()
	setup_ui()
...
func generate_random_numbers() -> void:
	"""Generate random strawberry counts ensuring minimum sum of 8"""
	...
	while not valid_combination:
		...
		# Check if sum is at least 8
		if random_left_count + random_right_count >= min_total_sum:
			valid_combination = true
	
	print("Random left strawberries: %d" % random_left_count)
	print("Random right strawberries: %d" % random_right_count)
...
# === STRAWBERRY NODE COLLECTION ===
func collect_strawberry_nodes() -> void:
	"""Collect all strawberry nodes from both groups"""
	left_strawberries.clear()
	right_strawberries.clear()
	
	for strawberry_path in left_strawberry_paths:
		var strawberry_node = get_node_or_null(strawberry_path)
		if strawberry_node:
			left_strawberries.append(strawberry_node)
			print("Found left strawberry: %s" % strawberry_path)
		else:
			print("Missing left strawberry: %s" % strawberry_path)
	
	for strawberry_path in right_strawberry_paths:
		var strawberry_node = get_node_or_null(strawberry_path)
		if strawberry_node:
			right_strawberries.append(strawberry_node)
			print("Found right strawberry: %s" % strawberry_path)
		else:
			print("Missing right strawberry: %s" % strawberry_path)
	
	print("Left strawberries: %d, Right strawberries: %d" % [left_strawberries.size(), right_strawberries.size()])

func display_strawberries_based_on_random() -> void:
	"""Show only the random amount of strawberries in shuffled positions"""
	# Collect all left strawberry indices
	var left_indices = []
	for i in range(left_strawberries.size()):
		left_indices.append(i)
	left_indices.shuffle()
	
	# Collect all right strawberry indices
	var right_indices = []
	for i in range(right_strawberries.size()):
		right_indices.append(i)
	right_indices.shuffle()
	
	# Hide all strawberries first
	for strawberry in left_strawberries:
		strawberry.visible = false
	for strawberry in right_strawberries:
		strawberry.visible = false
	
	# Show only the random count from shuffled positions
	for i in range(random_left_count):
		if i < left_indices.size():
			left_strawberries[left_indices[i]].visible = true
	
	for i in range(random_right_count):
		if i < right_indices.size():
			right_strawberries[right_indices[i]].visible = true
	
	print("Displayed %d left strawberries (shuffled) and %d right strawberries (shuffled)" % [random_left_count, random_right_count])

# === MATH PROBLEM SETUP ===
func setup_math_problem() -> void:
	"""Create the math problem based on visible strawberry counts"""
...
func setup_ui() -> void:
	...
	if equation_label:
		equation_label.text = "Solve the strawberry: " + current_equation
