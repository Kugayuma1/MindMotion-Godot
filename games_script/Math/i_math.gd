# VISUAL MATH GAME - CLEAN VERSION FOR ICECREAM SCENE
...
# === INITIALIZATION ===
func _ready() -> void:
	print("🍦 Starting IceCream Math Game - Debug Mode")
...
func collect_ice_nodes() -> void:
	...
	for ice_path in left_ice_paths:
		...
		if ice_node: left_icecreams.append(ice_node)
	
	for ice_path in right_ice_paths:
		var ice_node = get_node_or_null(ice_path)
		if ice_node: right_icecreams.append(ice_node)
	
	print("🍦 Left ice: %d, Right ice: %d" % [left_icecreams.size(), right_icecreams.size()])

# === MATH PROBLEM SETUP ===
func setup_math_problem() -> void:
	var left_count = left_icecreams.size()
	var right_count = right_icecreams.size()
