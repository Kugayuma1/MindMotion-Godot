...
var current_question := "How many Jaguars are in the scene?"
...
# Jaguar container
@onready var jaguar_container = $Jaguars  

# List of Jaguar node names (fallback if container not used)
var jaguar_names = ["Jaguar1", "Jaguar2", "Jaguar3", "Jaguar4"]
...
func find_all_jaguars():
	jaguar_nodes.clear()
	
	# Method 1: From container
	if jaguar_container:
		for child in jaguar_container.get_children():
			jaguar_nodes.append(child)
			print("✅ Found jaguar in container: %s" % child.name)
	else:
		# Method 2: From predefined names
		for jaguar_name in jaguar_names:
			var jaguar_node = get_node_or_null(jaguar_name)
			if jaguar_node:
				jaguar_nodes.append(jaguar_node)
				print("✅ Found jaguar by name: %s" % jaguar_name)
			else:
				print("❌ Could not find jaguar: %s" % jaguar_name)
	
	print("Total jaguars found: %d" % jaguar_nodes.size())
	
	if jaguar_nodes.size() == 0:
		print("⚠️ No jaguars found! Using fallback count of 4")
		correct_answer = 4
	else:
		correct_answer = jaguar_nodes.size()

func setup_counting_game():
	if question_label:
		question_label.text = current_question
	print("Question: %s" % current_question)
	print("Correct answer: %d Jaguars" % correct_answer)
...
func _on_button_pressed(button_num: int):
	...
	if selected_answer == correct_answer:
		if question_label:
			question_label.text = "✅ Correct! There are " + str(correct_answer) + " Jaguars!"
		timer_active = false
		game_over(true)
	else:
		if question_label:
...
func update_timer() -> void:
	if countdown <= 0:
		timer_active = false
		if question_label:
			question_label.text = "⏱️ Time's up! Answer: " + str(correct_answer) + " Jaguars"
		game_over(false)
		return
...
func game_over(success: bool):
	...
	if has_node("Jaguar"): $Jaguar.visible = false
