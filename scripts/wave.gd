...
func _ready():
	...
	# Auto-start the waving activity
	start_waving_activity()
...
func load_current_user():
	...
	if not Global.is_authenticated():
		push_error("Waving scene loaded but user not authenticated!")
		return
	
	if current_user_type != "student":
		push_warning("Waving scene loaded for non-student user type: " + current_user_type)

func start_waving_activity():
	instruction_label.text = "Wave Both Hands"
	status_label.text = "Get ready to show your waving skills!"
	
	# Wait 2 seconds then start detection
	await get_tree().create_timer(2.0).timeout
	
	if current_user_type == "student":
		motion_manager.start_motion_detection("wave", current_user_id)
		session_timer.start()
	else:
...
func _on_motion_session_started(motion_type: String):
	instruction_label.text = "Wave Both Hands"
	status_label.text = "Open Motion Detector app on your phone and start waving!"
	print("Motion detection started - waiting for waving...")
...
func finish_scene():
	...
	print("Waving scene finished - ready to continue game")
	
	# After waving is done, go to next level
