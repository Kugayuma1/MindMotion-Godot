...
func _ready():
	...
	# Auto-start the marching activity
	start_marching_activity()
...
func load_current_user():
	...
	if not Global.is_authenticated():
		push_error("Marching scene loaded but user not authenticated!")
		return
	
	if current_user_type != "student":
		push_warning("Marching scene loaded for non-student user type: " + current_user_type)

func start_marching_activity():
	instruction_label.text = "March In Place"
	status_label.text = "Get ready to march!"
	
	# Wait 2 seconds then start detection
	await get_tree().create_timer(2.0).timeout
	
	if current_user_type == "student":
		motion_manager.start_motion_detection("march", current_user_id)
		session_timer.start()
	else:
...
func _on_motion_session_started(motion_type: String):
	instruction_label.text = "March In Place"
	status_label.text = "Open Motion Detector app and start marching!"
	print("Motion detection started - waiting for marching...")

func _on_motion_detected():
	session_timer.stop()
	
	instruction_label.text = "Outstanding!"
	status_label.text = "Perfect marching detected!"
...
func finish_scene():
	...
	print("Marching scene finished - ready to continue game")
	
	# After marching is done, go to next level
