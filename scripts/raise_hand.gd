...
func _ready():
	...
	var custom_font = load("res://font/Summary Notes.ttf")
	...
	# Auto-start the raise hand activity
	start_raise_hand_activity()
...
func load_current_user():
	...
	if not Global.is_authenticated():
		push_error("Raise hand scene loaded but user not authenticated!")
		return
	
	if current_user_type != "student":
		push_warning("Raise hand scene loaded for non-student user type: " + current_user_type)

func start_raise_hand_activity():
	instruction_label.text = "Raise Your Hand"
	status_label.text = "Get ready to raise your hand!"
	
	# Wait 2 seconds then start detection
	await get_tree().create_timer(2.0).timeout
	
	if current_user_type == "student":
		motion_manager.start_motion_detection("raise_hand", current_user_id)
		session_timer.start()
	else:
...
func _on_motion_session_started(motion_type: String):
	instruction_label.text = "Raise Your Hand"
	status_label.text = "Open Motion Detector app and raise either hand!"
	print("Motion detection started - waiting for hand raising...")

func _on_motion_detected():
	session_timer.stop()
	
	instruction_label.text = "Excellent!"
	status_label.text = "Perfect hand raising detected!"
...
func finish_scene():
	...
	print("Raise hand scene finished - ready to continue game")
	
	# After raising hand is done, go to next level
